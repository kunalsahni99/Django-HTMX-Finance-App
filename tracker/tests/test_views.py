...
@pytest.mark.django_db
def test_start_and_end_date_filter(user_transactions, client):
    user = user_transactions[0].user
    client.force_login(user)
...
@pytest.mark.django_db
def test_cannot_add_transaction_with_zero_or_negative_amount(user, transaction_dict_params, client):
    client.force_login(user)
    user_transaction_count = Transaction.objects.filter(user=user).count()
